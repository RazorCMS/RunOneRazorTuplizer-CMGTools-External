							...
							bool calculateMva) 
{
	...
	int nPV = 0;
	for(unsigned int k=0; k < allvtx.size() ; ++k) {
	  if(allvtx.at(k).isValid() && !allvtx.at(k).isFake()
	     && allvtx.at(k).ndof() > 4 && fabs(allvtx.at(k).z()) <= 24 && allvtx.at(k).position().Rho() <= 2
	     ) {
	    nPV++;
	  }
	}
